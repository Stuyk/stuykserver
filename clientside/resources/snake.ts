...
function gameControls() {
    // Controls
    // W
    if (API.isDisabledControlPressed(Enums.Controls.MoveUpOnly)) {
        direction = 0;
    }
    // A
    if (API.isDisabledControlPressed(Enums.Controls.MoveLeftOnly)) {
        direction = 1;
    }
    // S
    if (API.isDisabledControlPressed(Enums.Controls.MoveDownOnly)) {
        direction = 2;
    }
    // D
    if (API.isDisabledControlPressed(Enums.Controls.MoveRightOnly)) {
        direction = 3;
    }
}
